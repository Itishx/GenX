...
// In-memory cost tracking (persists during server session)
let totalCostINR = 0;
let requestCount = 0;
...
export class ChatController {
    ...
    public async handleChat(req: Request, res: Response): Promise<void> {
        try {
            ...
            // Determine which OS and get stage details (with proper typing)
...
Help them make real progress on ${stage}. Get straight to actionable insights.`
            ...
            } else if (isLaunchOS && stageDetails) {
                systemPrompt = {
                    role: 'system',
...
╠─────────────────────────────────────────────────────────────╣
║ 💰 THIS REQUEST:
║   • Input cost:         ${formatCost(inputTokens * PRICING[model as keyof typeof PRICING].input)}
║   • Output cost:        ${formatCost(outputTokens * PRICING[model as keyof typeof PRICING].output)}
║   • Request total:      ${formatCost(requestCost)}
╠─────────────────────────────────────────────────────────────╣
║ 📈 CUMULATIVE COST:
║   • Total spent:        ${formatCost(totalCostINR)}
║   • Requests made:      ${requestCount}
║   • Average per req:    ${formatCost(totalCostINR / requestCount)}
╚═════════════════════════════════════════════════════════════╝
                `);
