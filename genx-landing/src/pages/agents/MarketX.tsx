  ...
  React.useEffect(() => {
    const load = async () => {
      ...
      const { data, error } = await supabase
        ...
        .eq('slug', 'marketx')
        .maybeSingle()
      if (error) console.warn('Fetch MarketX config error:', error)
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <div className="mx-auto max-w-4xl p-4">
        <motion.h1 initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="text-xl font-semibold">MarketX Dashboard</motion.h1>
        ...
        <div className="mt-4 flex gap-2">
          ...
          <button onClick={() => navigate('/app/chat?agent=marketx')} className="rounded-md bg-white px-3 py-2 text-sm font-semibold text-black">Go to Workspace (Chat)</button>
        </div>
      </div>
    </div>
  )
