  ...
  React.useEffect(() => {
    const load = async () => {
      ...
      const { data, error } = await supabase
        ...
        .eq('slug', 'codex')
        .maybeSingle()
      if (error) console.warn('Fetch CodeX config error:', error)
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <div className="mx-auto max-w-4xl p-4">
        <motion.h1 initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="text-xl font-semibold">CodeX Dashboard</motion.h1>
        <div className="mt-3 rounded-xl border border-white/10 bg-white/[0.03] p-4">
          {loading ? (
            ...
            <div className="space-y-2 text-sm">
              ...
              {agent.config?.devLevel && <div><span className="text-zinc-400">Experience:</span> {agent.config.devLevel}</div>}
        ...
        <div className="mt-4 flex gap-2">
          ...
          <button onClick={() => navigate('/app/chat?agent=codex')} className="rounded-md bg-white px-3 py-2 text-sm font-semibold text-black">Go to Workspace (Chat)</button>
        </div>
      </div>
    </div>
  )
