...
const DietXDashboard: React.FC<{
  ...
  const profession = agent?.config?.profession ?? 'Wellness'
  const purpose = agent?.config?.purpose ?? 'Nutrition guidance and meal plans.'

  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-white/10 text-white"><FiHeart className="h-5 w-5" /></span>
          <div>
            <div className="text-lg font-semibold text-white">{agent.name || 'DietX'}</div>
            <div className="text-xs text-zinc-400">Health and nutrition dashboard</div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <Section title="Daily Tips">
          <div>• Hydration reminder (coming soon)</div>
          <div>• Macro targets (coming soon)</div>
        </Section>
        <Section title="Integrations">
          <div>Health apps (coming soon)</div>
        </Section>
      </div>
    </motion.div>
  )
