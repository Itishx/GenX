...
import { FiArrowRight, FiCheckCircle, FiCircle, FiClock, FiChevronDown, FiMove } from 'react-icons/fi'
...
const KanbanColumn: React.FC<{
  ...
  const statusConfig = {
    'not-started': { icon: FiCircle, color: '#999999' },
    'in-progress': { icon: FiClock, color: '#3b82f6' },
    'done': { icon: FiCheckCircle, color: '#16a34a' },
  }
...
const DragDropInstructions: React.FC = () => {
    ...
    <>
      {!isOpen && (
        <motion.button
          ...
          className="mb-6 p-3 rounded-full bg-white border-2 border-blue-200 hover:border-blue-400 hover:bg-blue-50 transition-all duration-300 flex items-center justify-center group"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -8 }}
            transition={{ duration: 0.3 }}
            className="mb-6 p-6 rounded-lg bg-blue-50 border border-blue-200"
          >
            <div className="flex items-start justify-between gap-4 mb-4">
              <p className="text-base text-blue-900 font-semibold">💡 How to use this board:</p>
              <button
            ...
            <ul className="text-sm text-blue-800 space-y-2 ml-4">
              <li>• <strong>Drag & Drop:</strong> Grab the handle icon and drag stages between columns to move them.</li>
              <li>• <strong>Change Status:</strong> Click the status badge on any stage card to manually update it.</li>
              <li>• <strong>Reorder:</strong> Drag stages up or down within the same column to organize them.</li>
            </ul>
          </motion.div>
        )}
      </AnimatePresence>
...
const LaunchOSGetStarted: React.FC = () => {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#fafafa]">
      ...
      <main className="flex-1 pt-20">
        {/* Background pattern */}
        <div className="absolute inset-0 top-20 bg-[radial-gradient(#e8e8e8_0.5px,transparent_0.5px)] [background-size:24px_24px] opacity-20 pointer-events-none" />

        <section className="relative py-12 px-6 md:px-8">
          <div className="max-w-7xl mx-auto">
            ...
            >
              ...
              <p className="text-base text-[#666666] max-w-2xl leading-relaxed">
                Your strategic workspace for go-to-market clarity, messaging precision, and launch velocity.
              </p>
            </motion.div>
...
export default LaunchOSGetStarted
